...
class CRF(nn.Module):
    ...
    def viterbi(self, features):
        ...
        back_points, index_points = [self.transitions[self.start_tag, :self.start_tag].to(DEVICE)], [torch.LongTensor([-1]).expand_as(forward).to(DEVICE)]
